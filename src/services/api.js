...
export const QuizAPI = {
  // Pertanyaan quiz (hardcoded di frontend — backend gak punya endpoint GET /quiz)
  getQuestions: async () => {
    return {
      data: {
        questions: [
          ...
          },
          {
            ...
            options: [
              { value: 1, label_id: "Sangat tidak ada (1)" },
              { value: 2, label_id: "Kurang (2)" },
              { value: 3, label_id: "Cukup (3)" },
              { value: 4, label_id: "Baik (4)" },
              { value: 5, label_id: "Sangat baik (5)" }
            ]
...
export const ScanAPI = {
  // POST /scan/submit — multipart form with "photo" field
...
export const HistoryAPI = {
  getHistory: async () => {
    try {
      const [quizRes, scanRes] = await Promise.all([
        QuizAPI.getHistory(),
        ScanAPI.getHistory(),
      ]);

      const quizData = (quizRes.data.data || []).map(item => ({ ...item, type: 'quiz' }));
      const scanData = (scanRes.data.data || []).map(item => ({ ...item, type: 'scan' }));
